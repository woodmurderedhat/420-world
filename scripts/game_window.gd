...
func _ready() -> void:
	...
	$TitleBar/MinButton.pressed.connect(func():
		if _is_minimized:
...
func apply_palette(palette: Dictionary) -> void:
	...
	# The original code used self_modulate on the HBoxContainer? HBox doesn't draw bg unless it's a PanelContainer.
	# But GameWindow is a Panel, TitleBar is HBoxContainer.
	# If TitleBar is transparent HBox, we rely on GameWindow panel or a specific bg rect.
	...
	# HBoxContainer doesn't support stylebox override directly in Godot 4 unless replaced by PanelContainer.
	# BUT `self_modulate` on HBoxContainer only affects children if they inherit? No.
	# Wait, HBoxContainer does NOT draw a background. setting self_modulate does nothing unless it has a texture?
	...
	var btns: Array = [$TitleBar/CloseButton, $TitleBar/MinButton, $TitleBar/MaxButton, $TitleBar/FullButton]
	...
	if content_root.get_child_count() > 0:
		var app_root := content_root.get_child(0)
		if app_root != null:
			if app_root.has("metadata"):
				var m := app_root.get("metadata")
...
func _apply_icon_or_text(btn: Button, key: String, fallback_text: String) -> void:
	...
	btn.add_theme_color_override("font_color_pressed", _palette.get("accent", Color(0.2,0.6,1.0)).darkened(0.2))
	btn.add_theme_stylebox_override("normal", StyleBoxEmpty.new())
	btn.add_theme_stylebox_override("hover", StyleBoxEmpty.new())
	btn.add_theme_stylebox_override("pressed", StyleBoxEmpty.new())
	btn.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	var settings_node := get_tree().root.get_node_or_null("/root/SettingsManager")
	var icons_map: Dictionary = {}
	if settings_node != null:
		icons_map = settings_node.get_value("ui.window_icons", {}) as Dictionary
	var path := String(icons_map.get(key, ""))
