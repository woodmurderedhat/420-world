...
func refresh_global() -> void:
	...
	Log.info(
		(
			"[SaveManager] Global save loaded (schema=%d, keys=%d)"
			% [
				int(_global_cache.get("schema_version", SCHEMA_VERSION)),
				_global_cache.keys().size()
			]
		)
	)
...
func load_app(app_id: String) -> Dictionary:
	_ensure_apps_dir()
	var parsed: Dictionary = _load_json(_app_path(app_id), _default_app())
...
func delete_app_save(app_id: String) -> void:
	var path: String = _app_path(app_id)
	if FileAccess.file_exists(path):
		DirAccess.remove_absolute(path)
	var bak: String = "%s.bak" % path
...
func export_debug_bundle() -> bool:
	...
	var err: int = zip.open(DEBUG_BUNDLE_PATH)
	if err != OK:
	...
	if DirAccess.dir_exists_absolute(APPS_SAVE_DIR):
		var dir: DirAccess = DirAccess.open(APPS_SAVE_DIR)
		dir.list_dir_begin()
		var file_name: String = dir.get_next()
		while file_name != "":
			if not dir.current_is_dir() and file_name.ends_with(".json"):
				var path: String = "%s/%s" % [APPS_SAVE_DIR, file_name]
				zip.add_file(_read_bytes(path), _strip_user_prefix(path))
				var bak: String = "%s.bak" % path
				if FileAccess.file_exists(bak):
		...
		dir.list_dir_end()
	# Manifests
	var mdir: DirAccess = DirAccess.open("res://apps")
	if mdir != null:
		mdir.list_dir_begin()
		var entry: String = mdir.get_next()
		while entry != "":
			if mdir.current_is_dir() and not entry.begins_with("."):
				var mpath: String = "res://apps/%s/manifest.json" % entry
...
func _load_json(path: String, fallback: Dictionary) -> Dictionary:
	...
	var parsed: Dictionary = _try_load_json(path)
	if parsed.is_empty():
		var bak := "%s.bak" % path
...
func _write_json_with_backup(path: String, data: Dictionary) -> bool:
	if FileAccess.file_exists(path):
		var existing: PackedByteArray = _read_bytes(path)
		if existing.size() > 0:
			var bak: String = "%s.bak" % path
			var fb: FileAccess = FileAccess.open(bak, FileAccess.WRITE)
			if fb != null:
...
func _migrate_app(_app_id: String, data: Dictionary) -> Dictionary:
	var migrated: Dictionary = _default_app()
	for k in data.keys():
		migrated[k] = data[k]
