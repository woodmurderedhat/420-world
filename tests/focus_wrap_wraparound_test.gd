...
func _run_tests() -> void:
	...
	for i in range(6):
		...
		if not r["ok"]:
			print("Focus wrap wraparound test failed: couldn't create char %d" % i)
			quit(1)

	var cc = load("res://apps/character_creator/character_creator.tscn").instantiate()
	# enable wrap-around behavior (use set to avoid script/property binding edge-cases)
	...
	if grid == null:
		print("Focus wrap wraparound test failed: gallery grid missing")
		quit(1)
	var cards = []
	for ch in grid.get_children():
		if ch.name.begins_with("char_card_"):
			cards.append(ch)
	if cards.size() < 2:
		print("Focus wrap wraparound test inconclusive: less than 2 cards")
		quit(0)

	# Focus last card and press Right (should wrap to first)
	cards[-1].grab_focus()
	await get_tree().process_frame
	...
	if focused == null or not focused.name.begins_with("char_card_"):
		print("Focus wrap wraparound test failed: focus lost after right arrow")
		quit(1)
	if focused != cards[0]:
		print("Focus wrap wraparound test failed: expected wrap to first, but moved to %s" % focused.name)
		quit(1)

	# Focus first and press Left (should wrap to last)
	cards[0].grab_focus()
	await get_tree().process_frame
	...
	if focused != cards[-1]:
		print("Focus wrap wraparound test failed: expected wrap to last, but moved to %s" % str(focused))
		quit(1)

	print("Focus wrap wraparound test passed (wrap-around behavior)")
	quit(0)
